...
from src.models import Arrival, DemandPeak, Report, SourceStatus, TimeBlock
...
def build_now_report(
    ...
    f = _window(flights, now, window_end)
    t = _window(trains, now, window_end)
...
def build_tomorrow_report(
    ...
    all_arrivals = sorted(morning_flights + trains, key=lambda a: a.effective_time)
    blocks = _group_into_blocks(all_arrivals, tomorrow)
    return Report(
        generated_at=now,
...
def _window(arrivals: list[Arrival], start: datetime, end: datetime) -> list[Arrival]:
    return sorted(
...
def _peaks(arrivals: list[Arrival], location: str) -> list[DemandPeak]:
    ...
    for a in arrivals:
        slot = a.effective_time.strftime("%H:") + ("00" if a.effective_time.minute < 30 else "30")
        slots[slot] += 1
    return [
        DemandPeak(time_slot=s, count=c, location=location)
        for s, c in slots.most_common(3) if c >= 2
    ]


def _group_into_blocks(arrivals: list[Arrival], day: datetime) -> list[TimeBlock]:
    blocks: list[TimeBlock] = []
    for label, sh, eh in TIME_BLOCKS:
        bs = day.replace(hour=sh, minute=0, second=0)
        be = day.replace(hour=min(eh, 23), minute=59, second=59)
        items = [a for a in arrivals if bs <= a.effective_time <= be]
...
def _recs_tomorrow(
    ...
    if trains:
        recs.append(f"Trains all day: {len(trains)} scheduled")
    ...
    if not recs:
        recs.append("No schedule data for planning")
    return recs
