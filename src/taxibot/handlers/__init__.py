...
async def cmd_tgv(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await _handle_tgv_schedule(update, context)
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"📡 Trains  : GTFS + real-time delays (GTFS-RT)\n"
...
async def _handle_next_tgv(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ Could not load next TGV. Please try again in a moment.",
            reply_markup=KEYBOARD,
        )


async def _handle_tgv_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    pipeline = context.bot_data.get("pipeline")
    if pipeline is None:
        await update.message.reply_text("⚠️ Bot not ready yet, please try again.")
        return
    msg = await update.message.reply_text("⏳ Fetching TGV schedule…")
    try:
        ...
        await msg.delete()
        await update.message.reply_text(
            "❌ Could not load TGV schedule. Please try again in a moment.",
            reply_markup=KEYBOARD,
        )
