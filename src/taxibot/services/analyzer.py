...
def _recs_now(
    ...
    recs: list[str] = []
    if flights:
        f0, f1 = flights[0].effective_time.strftime("%H:%M"), flights[-1].effective_time.strftime("%H:%M")
        recs.append(f"{f0}–{f1} → Airport ({len(flights)} flight{'s' if len(flights) != 1 else ''})")
    if trains:
        t0, t1 = trains[0].effective_time.strftime("%H:%M"), trains[-1].effective_time.strftime("%H:%M")
    ...
    if not flights and not trains:
        ...
        recs.append(f"Quiet window — {', '.join(hints)}" if hints else "No arrivals scheduled — rest or reposition")
    return recs
